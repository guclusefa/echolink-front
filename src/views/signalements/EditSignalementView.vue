...
const initializeMap = async () => {
  try {
    ...
    const lat = latitude.value ?? 46.603354;
    const lng = longitude.value ?? 1.888334;
    const zoom = latitude.value && longitude.value ? 13 : 6;

    const leafletMap = L.map('edit-map').setView([lat, lng], zoom);
...
<template>
  <WrapperElement>
    <div class="max-w-4xl mx-auto">
      <h1 class="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Modifier le signalement</h1>
      ...
      <form @submit.prevent="updateSignalement" class="space-y-6">
        ...
        <div class="flex justify-end gap-4 pt-6">
          ...
          <ButtonElement type="submit" primary> Enregistrer les modifications </ButtonElement>
        </div>
      </form>
    </div>
  </WrapperElement>
