  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      const a =
        ...
        Math.cos(userLat * (Math.PI / 180)) * Math.cos(props.signalement.latitude * (Math.PI / 180)) *
...
const getPriorityBannerClass = (priority: string) => {
  return PRIORITY_BANNER_CLASSES[priority as keyof typeof PRIORITY_BANNER_CLASSES] || PRIORITY_BANNER_CLASSES.low;
};
    ...
    <div class="relative">
      ...
      <div class="p-4 pt-5">
        <div class="flex items-start justify-between gap-2">
          <h3 class="text-lg font-medium line-clamp-2 flex-1">{{ props.signalement.description }}</h3>
        ...
        <div class="mt-3 flex flex-wrap gap-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          <span v-if="props.signalement.userName && props.signalement.userLastName" class="flex items-center">
